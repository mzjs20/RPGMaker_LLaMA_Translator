...
def translate_text(text, index, version, repetitive_part=None, attempt=1):
    if attempt > 3:
        # 如果重试次数超过3次，跳过这一行
    ...
    if version == '0.8':
        ...
        # query = "将下面的日文文本翻译成中文：" + modified_text
        data = {
            "frequency_penalty": 0,
            "n_predict": 1000,
    ...
    # 发送POST请求
    response = requests.post("http://127.0.0.1:6006/completion", json=data)
    # 获取响应的内容
...
def main(version):
    ...
    for i in tqdm(range(start_index, len(keys)), desc="翻译进度"):
        ...
        if contains_jp:
            ...
            translated_text = translate_text(updated_text, i, version, repetitive_part)
